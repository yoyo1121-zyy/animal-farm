...
// HTML5 Web版本
console.log('[scene-3d.js] 文件开始执行...');

// THREE.js 将从 window.THREE 动态获取（运行时检查）
let createScopedThreejs = null;

// 加载物理世界（可选）
let PhysicsWorld;
try {
  if (typeof window.PhysicsWorld !== 'undefined') {
...
let ModelConfig3D;
try {
  if (typeof window.ModelConfig3D !== 'undefined') {
    ModelConfig3D = window.ModelConfig3D;
  }
} catch (e) {
...
class Scene3D {
  ...
  async init(canvas) {
    ...
    try {
      ...
      // 2. 初始化 Three.js (Web环境使用全局THREE)
      if (typeof window.THREE !== 'undefined') {
        console.log('[Scene3D] 使用全局 THREE.js (Web环境)');
        this.THREE = window.THREE;
      } else if (createScopedThreejs) {
        console.log('[Scene3D] 使用 threejs-miniprogram (微信小游戏环境)');
        this.THREE = createScopedThreejs(canvas);
      } else {
        console.error('[Scene3D] THREE.js 不可用');
        return false;
      }
...
// Web环境导出
console.log('[scene-3d.js] 开始导出...');
if (typeof module !== 'undefined' && module.exports) {
  module.exports = Scene3D;
} else {
  if (typeof window !== 'undefined') {
    window.Scene3D = Scene3D;
    console.log('[scene-3d.js] 导出到 window.Scene3D');
  }
