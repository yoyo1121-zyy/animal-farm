...
function loadModule(moduleName, globalName) {
  ...
  console.warn(`无法加载模块: ${moduleName} (${globalName})`);
  return null;
}

// 从全局对象加载模块（模块已在各自的文件中声明并导出到window）

// 加载 3D 渲染器
let Scene3DModule = null;
let ThreeSceneModule = null;
let Simple3DRendererModule = null;

// 优先尝试加载 Scene3D (Three.js + Cannon.js 完整3D方案)
try {
  Scene3DModule = loadModule('./scene-3d.js', 'Scene3D');
  if (Scene3DModule) {
...
if (!Scene3DModule) {
  try {
    ThreeSceneModule = loadModule('./three-scene.js', 'ThreeScene');
    if (ThreeSceneModule) {
...
if (!Scene3DModule && !ThreeSceneModule) {
  try {
    Simple3DRendererModule = loadModule('./simple-3d-renderer.js', 'Simple3DRenderer');
    if (Simple3DRendererModule) {
...
class Game {
  constructor() {
    ...
    this.canvasUI.id = 'gameUICanvas'; // 设置唯一 ID，方便调试
    ...
    // ========== 帧序列动画背景 + 音频 ==========
    this.frameAnimation = new window.FrameAnimation();
    // 帧序列路径：images/bg/frame_001.png, frame_002.png, ... (三位数编号)
    const framePattern = 'miniprogram/images/bg/frame_{n:03d}.png';
    const frameCount = 119; // 总帧数
    const audioPath = 'miniprogram/images/animal-farm.mp3';
  ...
  async init3DScene() {
    ...
    if (Scene3DModule) {
      try {
        ...
        this.threeScene = new Scene3DModule();
        // 使用 WebGL canvas 初始化
    ...
    if (ThreeSceneModule) {
      try {
        ...
        this.threeScene = new ThreeSceneModule();
    ...
    if (Simple3DRendererModule) {
      try {
        ...
        this.threeScene = new Simple3DRendererModule();
        const success = await this.threeScene.init();
        if (success) {
          this.use3DModels = true;
          this.useFull3D = false;
  ...
  triggerShake(rotationSpeed = 2.0) {
    ...
    if (this.physicsEnabled && this.physicsManager) {
      ...
      // 重新计算可点击状态
      this.gameData.items = window.GameLogic.updateClickable(this.gameData.items);
    }

    // ========== 颠勺可能触发补充物品 ==========
    // 使用统一的补充逻辑
    const config = window.GameLogic.ITEM_COUNT_CONTROL;
    const needsRefill =
  ...
  drawEliminateAnimation(anim, progress) {
    const ctx = this.ctxUI || this.ctx; // 优先使用UI canvas
    if (!ctx) return;
  ...
  drawLevel1Background() {
    ...
    if (!this.level1BgImage) {
      ...
      this.level1BgImage.src = 'miniprogram/images/bg-level-1.png';
    }
  ...
  drawLevel2Background() {
    ...
    if (!this.level2BgImage) {
      ...
      this.level2BgImage.src = 'miniprogram/images/bg-level-2.png';
    }
  ...
  drawLevel3Background() {
    ...
    if (!this.level3BgImage) {
      ...
      this.level3BgImage.src = 'miniprogram/images/bg-level-3.png';
    }
  ...
  drawLevel4Background() {
    ...
    if (!this.level4BgImage) {
      ...
      this.level4BgImage.src = 'miniprogram/images/bg-level-4.png';
    }
  ...
  loadImages() {
    ...
    for (let i = 1; i <= 10; i++) {
      levelIconPromises.push(loadSingleImage(`level-${i}`, `miniprogram/images/level-icons/level-${i}.png`));
    }

    Promise.all([
      loadSingleImage('background', 'miniprogram/images/homepage-bg.png'),
      loadSingleImage('star-filled', 'miniprogram/images/star-filled.png'),
      loadSingleImage('star-empty', 'miniprogram/images/star-empty.png')
  ...
  loadGameData() {
    try {
      // 从本地存储读取已解锁关卡，默认为1（只有第1关解锁）
      this.maxLevel = Math.min(wx.getStorageSync('maxLevel') || 1, 4);
  ...
  handleHomeTouch(x, y) {
    ...
    // 关卡选择 - 与渲染位置保持一致（4关，1行4列）
    const levelCount = 4;
    const levelsPerRow = 4; // 4列
  ...
  checkMatchType() {
    ...
    for (const key in itemCounts) {
      if (itemCounts[key] >= 3) {
        ...
        const parts = key.split('_');
        // 确保颜色以 # 开头（处理可能的前缀丢失问题）
        let color = parts.slice(0, -1).join('_');
  ...
  useProp(propKey) {
    ...
    switch (propKey) {
      ...
      case 'shuffle':
        this.gameData.items = window.GameLogic.shuffleItems(this.gameData.items);
  ...
  async startGame() {
    ...
    // 立即设置场景为 'game'，防止渲染循环调用 renderHome
    this.scene = 'game';
    ...
    if (this.canvas2D) {
      this.canvas2D.style.display = 'block';
      this.canvas2D.style.zIndex = '0'; // 最底层
      console.log('[startGame] 显示 canvas2D:', this.canvas2D.id);
    }

    // 显示 UI canvas（用于绘制物品和 UI）
    if (this.canvasUI) {
      this.canvasUI.style.display = 'block';
      this.canvasUI.style.zIndex = '10'; // 最上层，确保物品和 UI 可见
      console.log('[startGame] 显示 canvasUI:', this.canvasUI.id);
    }

    // 重新显示 webglCanvas（3D 渲染）
    const webglCanvas = document.getElementById('webglCanvas');
    if (webglCanvas) {
    ...
    // 隐藏 HTML 中的 uiCanvas（避免与 this.canvasUI 冲突）
    const htmlUiCanvas = document.getElementById('uiCanvas');
    if (htmlUiCanvas) {
      htmlUiCanvas.style.display = 'none';
    }

    // ========== 手动解锁音频（用户已点击开始按钮）==========
    console.log('[startGame] 手动解锁音频');
    ...
    const config = window.GameLogic.getLevelConfig(this.currentLevel);
    const phase = 'early'; // 初始阶段

    // 只生成初始数量的物品（约40个）
    const initialSpawnCount = config.initialSpawnCount || 40;
    // 传入主题ID
    const items = window.GameLogic.generateGameItems(config, phase, initialSpawnCount, config.theme.id);

    // 初始化物理引擎
    if (this.physicsManager) {
      this.physicsManager.destroy();
    }
    this.physicsManager = new window.PhysicsManager(this.width, this.height);
    ...
    // 更新可点击状态
    const updatedItems = window.GameLogic.updateClickable(items);
  ...
  async refillFromBottom() {
    ...
    const phase = window.GameLogic.getCurrentPhase(progress);
    ...
    const phaseConfig = window.GameLogic.PHASE_CONFIG[phase];
    const pushItems = [];

    // 计算需要生成多少组
    const groupsToSpawn = refillCount / 3;

    // 移除"三消保障"逻辑，让物品随机弹出，增加难度
    // 但确保每个类型的物品总数是3的倍数（在生成关卡时已保证）

    for (let g = 0; g < groupsToSpawn; g++) {
      let typeId;
      let selectedColor = null;

      console.log(`[refillFromBottom] 开始生成第${g+1}组物品（随机）`);

      // ========== 随机选择物品类型（不故意配对）==========
      // 确保 existingTypes 不为空
      if (existingTypes.length === 0) {
        // 如果场上没有任何物品，使用关卡配置的物品类型
        const levelConfig = window.GameLogic.getLevelConfig(this.currentLevel);
        existingTypes = levelConfig.itemTypes || [1]; // 默认使用类型1（奶牛）
      }
      typeId = existingTypes[Math.floor(Math.random() * existingTypes.length)];

      // 从所有主题中查找物品类型（直接查找，不依赖 findItemTypeById）
      let finalItemType = null;
      for (const themeKey in window.GameLogic.ITEM_TYPES) {
        const found = window.GameLogic.ITEM_TYPES[themeKey].find(t => t.id === typeId);
        if (found) {
          finalItemType = found;
          break;
        }
      ...
      // 如果找不到，使用默认类型（奶牛）
      if (!finalItemType || !finalItemType.emoji) {
        console.warn(`[refillFromBottom] 找不到typeId=${typeId}，使用奶牛`);
        typeId = 1;
        finalItemType = window.GameLogic.ITEM_TYPES.ranch.find(t => t.id === 1);
      }

      const finalSize = window.GameLogic.getItemSize(typeId);

      // 确定颜色（使用物品定义的颜色）
      if (!selectedColor) {
        selectedColor = finalItemType.color || '#CCCCCC';
      }
      ...
      for (let i = 0; i < 3; i++) {
        pushItems.push({
          ...
          emoji: finalItemType.emoji,
          color: selectedColor,
          size: finalSize,
          sizeConfig: window.GameLogic.ITEM_SIZE_CONFIG[finalSize]
        });
      }
    }
    ...
    pushItems.forEach((item, index) => {
      ...
      // ========== 设置弹跳初速度 ==========
      item.initialForce = {
        x: Math.cos(angle) * 0.15, // 向外扩散力度
        y: 0.5 + Math.random() * 0.3 // 向上弹起力度
      };
      item.initialAngularVel = (Math.random() - 0.5) * 0.05; // 随机旋转
    });
  ...
  gameOver(isWin) {
    ...
    // 隐藏 UI canvas
    if (this.canvasUI) {
      this.canvasUI.style.display = 'none';
    }

    // 隐藏 webglCanvas（H5 环境），确保游戏结束界面不被遮挡
    const webglCanvas = document.getElementById('webglCanvas');
    if (webglCanvas) {
      webglCanvas.style.display = 'none';
    }
    ...
    // 隐藏所有动态创建的 2D canvas
    const allCanvases = document.querySelectorAll('canvas[id^="canvas2D_"]');
    allCanvases.forEach((canvas, index) => {
      // 只保留当前使用的 2D canvas
      if (canvas !== this.canvas2D) {
  ...
  render() {
    ...
    if (this.scene === 'home') {
      ...
      // 不调用 renderGame()，避免 3D 场景遮挡游戏结束界面
      this.renderGameOver();
    }
  ...
  renderHome() {
    ...
    // 统计信息区域 - 纪念碑谷风格
    const statsY = this.height * 0.28 + 7;
    ...
    // ========== 关卡选择 - 纪念碑谷风格 ==========
    const levelCount = 4; // 只显示4关
    const levelsPerRow = 4; // 4关排成一行
  ...
  renderGame() {
    ...
    // ========== 保存原始 ctx 并设置正确的绘制上下文 ==========
    const originalCtx = this.ctx;

    // 绘制游戏页面背景（使用 2D canvas）
    // 在 3D 模式下，this.ctx 为 null，需要临时设置为 2D canvas 的上下文
    const bgCtx = this.canvas2D.getContext('2d');
    // 确保背景 canvas 的变换矩阵正确
    ...
    // 临时设置 this.ctx 为背景 canvas 上下文，用于绘制背景
    this.ctx = bgCtx;
    ...
    // ========== 设置 this.ctx 为 UI canvas，确保UI元素正确绘制 ==========
    this.ctx = this.ctxUI || this.ctx;

    // 确保 UI canvas 的变换矩阵正确
    if (this.ctxUI) {
      this.ctxUI.setTransform(this.dpr, 0, 0, this.dpr, 0, 0);
      // 清空UI canvas（只清空UI层，不影响背景）
      this.ctxUI.clearRect(0, 0, this.width, this.height);
    }

    // 顶部信息栏 - 纪念碑谷风格
    const topBarHeight = this.height * 0.12;
    const ctx = this.ctx; // 使用已设置的 this.ctx
    if (!ctx) return;
...
// Web环境导出
console.log('[game.js] 开始导出 Game 类...');
if (typeof module !== 'undefined' && module.exports) {
  module.exports = Game;
  console.log('[game.js] 导出到 module.exports');
} else {
  if (typeof window !== 'undefined') {
    window.Game = Game;
    console.log('[game.js] 导出到 window.Game');
  }
...
console.log('[game.js] Game 类导出完成');
